...
// Most singularly responsible component
// Board - the game
...
export default function Board(props: any) {
  ...
  function calcBoardPayout(board: string[][]): any[] {
    ...
    for (const line of PAYLINES) {
      const [c1, c2, c3] = line;
      const lineResult = extractPayline(board, c1, c2, c3);
      const multiResult = calcPaylineMulti(lineResult);
  
      multiResults.push(multiResult);

      // Something to highlight as a result on a payline
      if (multiResult !== 0) {
        console.log('idx', idx);
